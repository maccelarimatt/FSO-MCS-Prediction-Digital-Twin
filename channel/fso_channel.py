...
Gamma-Gamma distribution, with parameters derived from the Von Kármán
spectrum model (including outer scale and aperture averaging corrections).

Temporal statistics are governed by the frozen-flow hypothesis
(Taylor's hypothesis), implemented via an AR(1) process matched
to the atmospheric coherence time.

Method:
  1. Compute Gamma-Gamma shape parameters (α, β) from the Von Kármán
     Rytov variance with aperture averaging (see turbulence.py).
  2. Generate temporally correlated Gaussian noise via AR(1), matched
     to the coherence time τ₀ = 0.3 * r₀ / v_wind.
  3. Transform Gaussian marginals to Gamma-Gamma via inverse CDF.
  
This preserves the correct marginal distribution (Gamma-Gamma with
Von Kármán parameters) and first-order temporal correlation (matched
to frozen-flow coherence time). Higher-order temporal structure
(fade asymmetry, burst statistics) is not captured by the AR(1)
model — this is a known limitation documented in the thesis.
...
class FSOChannel:
    ...
    def _generate_correlated_gaussian(
        ...
        """
        Generate Gaussian noise with frozen-flow temporal correlation.
        
        The temporal covariance of intensity fluctuations for a
        receiver under the frozen-flow hypothesis approximately follows:
        
          B_I(τ) ∝ exp(-(τ/τ_0)^{5/3})
        
        We use an AR(1) approximation matched to the coherence time τ₀, which captures the essential
